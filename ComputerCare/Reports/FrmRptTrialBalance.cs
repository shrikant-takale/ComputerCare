...
{
    public partial class FrmRptTrialBalance : Form
    {
        connection c = new connection();
        SqlConnection con;
        string query, cname;
        string TrialBalance = Path.Combine(System.Windows.Forms.Application.StartupPath, "RptTrialBalance.rpt");
        DataTable dt = new DataTable();
        int billno;

        public FrmRptTrialBalance()
        {
            InitializeComponent();
        }

        private void FrmRptTrialBalance_Load(object sender, EventArgs e)
        {
            getCompanyName();
        }
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con = c.openConnection();
                query = "select * from tblbalancesheet s,tblowner o where date between @date1 and @date2 and s.oid=o.oid and o.oid='" + lblid.Text + "'  and s.year='" + lblYear.Text + "'";
                SqlCommand cmd = new SqlCommand(query, con);
        ...
        private void FrmRptTrialBalance_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
